...
$query = "SELECT * from `user` INNER JOIN `coach` ON `user`.`userID`=`coach`.`userID` INNER JOIN `school` ON `coach`.`schoolID`=`school`.`schoolID` $where";
$output .= printResults($mysqlConn, $query,0);

/*get students*/
$output .= "<h3>Students</h3>";
$query = "SELECT * from `user` INNER JOIN `student` ON `user`.`userID`=`student`.`userID` INNER JOIN `school` ON `student`.`schoolID`=`school`.`schoolID` $where";
...
if(userHasPrivilege(5))
{
	...
	$query = "SELECT * FROM `user` D WHERE NOT EXISTS(SELECT * FROM `student` S WHERE D.`userID` = S.`userID`) AND NOT EXISTS (SELECT * FROM `coach` C WHERE D.`userID` = C.`userID`)";
	$output .= printResults($mysqlConn, $query,0);
}
