...
function linkUser($userID, $email)
{
	...
	if ($result && $this->db->affected_rows) {
		error_log( "Linking Student");
		$queryPrivilege = "UPDATE `user` SET `privilege`= IF(`privilege`=0,1,`privilege`) WHERE `userID`=$userID";
		...
		$query = "UPDATE `coach` SET `userID` = $userID WHERE `email` LIKE '$email'";
		$result = $this->db->query($query) or error_log("\n<br />Warning: query failed:$query. " . $this->db->error. ". At file:". __FILE__ ." by " . $_SERVER['REMOTE_ADDR'] .".");
		if ($result && $this->db->affected_rows) {
			error_log( "Linking Coach");
			$queryPrivilege = "UPDATE `user` SET `privilege`= IF(`privilege`=0,1,`privilege`) WHERE `userID`=$userID";
			$result = $this->db->query($queryPrivilege) or error_log("\n<br />Warning: query failed:$queryPrivilege. " . $this->db->error. ". At file:". __FILE__ ." by " . $_SERVER['REMOTE_ADDR'] .".");
			return true;
		} else {
...
function checkUserType($userID, $email, $checkLinking)
{
	error_log( "user logging in:".$userID.", email:".$email);
	...
	if($result->num_rows > 0){
		...
		//try linking to account
		if ($checkLinking && $this->linkUser($userID, $email))
		{
			error_log( "Account linked just now for $userID");
			return $this->checkUserType($userID, $email, false);
		}
		else {
			...
			if($result->num_rows > 0){
				...
				if($row['privilege']>4)
				{
					return [0, 'super', 5]; //school id, type
				}
			}
...
function checkUser($data = array()){
	if(!empty($data)){
		...
		if($checkResult->num_rows > 0){
			...
			$userType = $this->checkUserType($checkRow['userID'], $checkRow['email'], true);
			...
			$query = "UPDATE `user` SET ".$colvalSet.$whereSql;
			...
			$this->linkUser($userID,  $data['email']);
